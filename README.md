...
To run it as a service, see [`packaging/`](packaging): a systemd user unit, a
macOS LaunchAgent, and a Prometheus Agent configuration. Release binaries for
all four platforms are attached to each GitHub release.
...
`git tag -a vX.Y.Z && git push --tags`. The workflow refuses a tag that
disagrees with `internal/version`, and a version with no section in
`docs/changelog.md`, whose section becomes the release notes.
