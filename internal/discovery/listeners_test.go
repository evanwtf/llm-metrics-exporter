...
func TestLocalListenersFindNewPort(t *testing.T) {
	if runtime.GOOS!="linux" && runtime.GOOS!="darwin" {t.Skip("unsupported OS")}
	l,err:=net.Listen("tcp4","127.0.0.1:0");if err!=nil {t.Fatal(err)};defer l.Close()
	ctx,cancel:=context.WithTimeout(context.Background(),2*time.Second);defer cancel()
	got,err:=LocalListeners(ctx);if err!=nil {t.Fatal(err)}
	for _,target:=range got {if target.URL=="http://"+l.Addr().String() {return}}
...
`
	got,err:=parseProc(strings.NewReader(input));if err!=nil {t.Fatal(err)}
	if len(got)!=2 || got[0].URL!="http://127.0.0.1:8080" || got[0].Generation!="123" {t.Fatalf("%+v",got)}
}
